...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title !== '' && $remarks !== '' && in_array($status, ['Pending', 'In Progress', 'Implemented', 'Rejected'])) {
        $stmt = $db->prepare("INSERT INTO admin_suggestions (admin_id, title, remarks, status) VALUES (?, ?, ?, ?)");
        $stmt->execute([$_SESSION['admin_id'], $title, $remarks, $status]);
...
<head>
    ...
    <title>Add Suggestion - Family Tree Admin</title>
                    ...
                    <form method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="remarks" name="remarks" rows="5" required
                                      placeholder="Describe the feature improvement or suggestion in detail"><?php echo isset($_POST['remarks']) ? htmlspecialchars($_POST['remarks']) : ''; ?></textarea>
                        </div>
                        <div class="mb-4">
                            ...
                            <select class="form-select" id="status" name="status">
                                ...
                                <option value="Implemented" <?php echo (isset($_POST['status']) && $_POST['status'] === 'Implemented') ? 'selected' : ''; ?>>Implemented</option>
